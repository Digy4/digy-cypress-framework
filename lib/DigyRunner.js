...
let DigyRunner = {
  ...
  setup: async (on, config) => {
    ...
    on('before:run', (spec) => {
      ...
      DigyRunner.init(spec, config.digyRunnerConfig);
    })
  ...
  init: async (spec, digyRunnerConfig) => {
    ...
    digyRunnerConfig.REGION = process.env.REGION;
    if (!(digyRunnerConfig.RESULTS_SUMMARY_URL && digyRunnerConfig.RESULTS_URL && digyRunnerConfig.REGION)) {
  ...
  sendResultSummaryForFinish: async (digyRunnerConfig, status, results) => {
    ...
    const s3Client = await DigyUtils.setupS3();
    await DigyUtils.uploadTerminalLogs(s3Client, digyRunnerConfig.BUILD_ID, DigyRunner.testResultSummary.startTime);
  },
